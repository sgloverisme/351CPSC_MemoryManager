...
void tlb_remove(int index) { 
    for(int i = index; i < PTABLE_SIZE; i++){
        //pg.table[i]=pg.table[index+1]; //shifting
        //pg.table[PTABLE_SIZE-1]=0;
    }//end of for loop
}  // TODO
...
void tlb_miss(size_t& frame, size_t& page, size_t& tlb_track) { 
   frame = pg_table[page].frame_num;
    //updating frame from the page table;
    //idea of tlb_miss is get the frame and update the tlb for the future
    //more to do need to update the tlb
} // TODO
